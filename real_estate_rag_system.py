...
class TemporalMemoryManager:
    ...
    def get_temporal_context(self, query: str, time_range: str = '1Y') -> List[Dict[str, Any]]:
    ...
    def _get_competitor_trends(self, start_date: datetime, end_date: datetime) -> List[Dict[str, Any]]:
        """Get competitor activity trends"""
    ...
    def compare_periods(self, metric: str, period1: str, period2: str) -> Dict[str, Any]:
        """Compare specific metrics across different time periods"""
        
        cache_key = f"{metric}_{period1}_{period2}"
        if cache_key in self.memory_cache:
            return self.memory_cache[cache_key]
        
        # Get data for both periods
        period1_data = self._get_period_data(metric, period1)
        period2_data = self._get_period_data(metric, period2)
        ...
        comparison = {
            ...
            'comparison': self._calculate_period_comparison(period1_data, period2_data)
        }
        
        # Cache result
        self.memory_cache[cache_key] = comparison
        self._save_memory()
        
        return comparison
    
    def _get_period_data(self, metric: str, period: str) -> Dict[str, Any]:
        """Get data for a specific time period"""
        
        # Parse period (e.g., "2024-Q1", "2023-12", "2024")
        if 'Q' in period:
            ...
            if end_month > 12:
                end_date = datetime(int(year) + 1, end_month - 12, 1) - timedelta(days=1)
            else:
                end_date = datetime(int(year), end_month + 1, 1) - timedelta(days=1)
        elif len(period) == 4:  # Year
        ...
        return {
            ...
            'min': min(values),
            'max': max(values),
            'trend': 'up' if values[-1] > values[0] else 'down' if len(values) > 1 else 'stable'
        }
    
    def _calculate_period_comparison(self, period1_data: Dict, period2_data: Dict) -> Dict[str, Any]:
        """Calculate comparison between two periods"""
        
        if period1_data['count'] == 0 or period2_data['count'] == 0:
            return {'status': 'insufficient_data'}
        
        avg_change = ((period2_data['average'] - period1_data['average']) / period1_data['average']) * 100
        
        return {
            'average_change_percent': round(avg_change, 2),
            'direction': 'increase' if avg_change > 0 else 'decrease',
            'significance': 'high' if abs(avg_change) > 10 else 'medium' if abs(avg_change) > 5 else 'low'
        }
...
class RealEstateRAGSystem:
    ...
    def _load_existing_data(self):
        ...
        for comp in competitor_data:
            content = f"Competitor Activity: {comp.company_name}. Transaction: {comp.recent_transaction}. Value: ${comp.transaction_value:,.0f} if comp.transaction_value else 'N/A'}. Market Share: {comp.market_share}%"
        ...
        logger.info(f"Added {len(documents)} new documents to RAG system")
...
Loaded {len(all_documents)} documents into RAG system")
    ...
    def _calculate_confidence(self, retrieved_docs: List[Dict[str, Any]], answer: str) -> float:
        ...
        confidence = (avg_retrieval_score * 0.6 + doc_count_factor * 0.2 + answer_length_factor * 0.2)
        
        return round(confidence, 2)
    
    def add_new_data(self, documents: List[Document]):
